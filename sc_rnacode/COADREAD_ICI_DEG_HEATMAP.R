...
genes <- read_csv('coadread_ici_diff_genes.csv')

gene_name <- genes %>% 
  select(`...1`) %>% 
  pull()

df <- FetchData(object=sc,vars=gene_name)# 231genes mergered
...
Heatmap(df_scale,cluster_rows = T,cluster_columns = F,show_column_names = F,
